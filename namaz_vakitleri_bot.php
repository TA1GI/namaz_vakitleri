<?php
// =============================================================
// NAMAZ VAKİTLERİ OTOMASYON BOTU (PROD SÜRÜM - v1.0)
// =============================================================

// AYARLAR
define('BATCH_LIMIT', 15); // Her çalışmada 15 ilçe indirir.
...
function archiveOldFiles() {
    ...
    foreach ($files as $file) {
        // Dosya zaten bir alt klasördeyse atla
        if (dirname($file) !== '.') continue;
        rename($file, $archive_folder . '/' . basename($file));
    }
    echo "[ARŞİV] Eski dosyalar arşivlendi.\n";
}
...
// Ocak ayındaysak içinde bulunduğumuz yılı indir (Örn: 2026)
if (date('m') == '01') {
...
foreach ($locations as $province => $districts) {
    foreach ($districts as $district_name => $district_id) {
        ...
        if ($downloaded_count >= BATCH_LIMIT) {
            echo "--------------------------------------------------\n";
            echo "[LİMİT] Kota doldu, işlem durduruluyor.\n";
            exit;
        }
