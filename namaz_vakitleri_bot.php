<?php
// =============================================================
// NAMAZ VAKİTLERİ OTOMASYON BOTU (GitHub Actions Uyumlu)
// =============================================================
...
function archiveOldFiles() {
    ...
    foreach ($files as $file) {
        ...
        rename($file, $archive_folder . '/' . $filename);
    }
    echo "[ARŞİV] Dosyalar '$archive_folder' içine taşındı.\n";
}
...
foreach ($locations as $province => $districts) {
    foreach ($districts as $district_name => $district_id) {
        ...
        if ($downloaded_count >= BATCH_LIMIT) {
            echo "--------------------------------------------------\n";
            echo "[LİMİT] $downloaded_count adet ilçe indirildi. Bir sonraki turda devam edilecek.\n";
            exit;
        }
        ...
        if ($html) {
            ...
            if ($data && count($data) > 300) {
                file_put_contents($filepath, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
                echo "OK (" . count($data) . " gün)\n";
                $downloaded_count++;
                sleep(rand(1, 2)); // Nezaketen bekle
            } else {
                echo "HATA (Veri eksik)\n";
            }
        } else {
            echo "HATA (Bağlantı)\n";
        }
